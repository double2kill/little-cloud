import type { RaceSeries } from "@little-cloud/data";
...
export function buildCalendarDays(
  year: number,
  month: number,
  raceSeriesByDate: Map<string, RaceSeries>,
  today: Date = new Date()
): CalendarDay[] {
  ...
  for (let i = startWeekday - 1; i >= 0; i--) {
    ...
    days.push({
      ...
      isRaceDay: raceSeriesByDate.has(date),
      series: raceSeriesByDate.get(date),
    });
  }
